...
The app reads from a catalog/schema that the companion DAB populates. Default location: `main.youtube_channels` (overridable via env vars).

| Table | Produced by | Purpose |
|---|---|---|
| `iab_viz_precomputed` | DAB job `precompute-viz` | 698 IAB categories + embeddings + t-SNE/UMAP coords + clusters |
| `channels_output` | DAB job `classify-channels-v2` | Classified channels (primary category, confidence, path) |
| `channels_prepped` | DAB job `classify-channels-v2` | Channel text used for embedding |
| `channels_embeddings` | DAB job `classify-channels-v2` | Channel embedding vectors |
| `channels_viz_sample` | DAB job `precompute-viz` | 5,000 channels with 3D coords for Galaxy view |
| `viz_dendrogram_linkage` | DAB job `precompute-viz` | Pre-computed Ward linkage for dendrogram |
...
> ⚠️ The resource name **`sql-warehouse`** must match the `valueFrom: 'sql-warehouse'` in `app.yaml` — that's how the warehouse ID gets injected as the `DATABRICKS_WAREHOUSE_ID` env var at process start. If you rename it, update `app.yaml` to match.
...
| `/api/embed` returns 404 on the endpoint | `databricks-gte-large-en` not available in your region | Edit `lib/embedding-client.js` line 8 — set `MODEL` to another embedding endpoint (e.g., `bge-large-en`). Also grant the SP `Can query`. |
...
| Swap | Where |
|---|---|
| Taxonomy table | `iab_viz_precomputed` → your taxonomy table with embeddings + 3D coords |
| Reference pool table | `channels_viz_sample` → items from your domain |
...
Update `CATALOG` / `SCHEMA` in `app.yaml` and redeploy. For per-environment overrides, create multiple app shells (e.g., `embeddings-explorer-dev`, `-prod`).
